...
import { CIDADE_LISTAR, USUARIO_ALTERAR, USUARIO_BUSCAR_POR_ID, USUARIO_LISTAR, USUARIO_SALVAR } from "../../config/urlSistema";
...
const Alterar = () => {
  ...
  const { errorAxios, getData, putData, getDataById } = useApi(); 
  const {  
    usuario,
    setUsuario,
    errors,
  ...
  const handleSubmitDadosUsuario = async (e) => {
     ...
     await putData(idUsuario, USUARIO_ALTERAR, usuario);
     if(!errorAxios){
       //setUsuario(USUARIO);
       //setRedirect(true);
     }
